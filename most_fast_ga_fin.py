...
def export_var(file_name,var1 = [],var2 = [],var3 = [],var4 = [],var5 = []): #結束輸出各極限值 txt檔
    ...
    elif file_name == "all_best_f1.txt":  
...
class GeneticAlgo:
    ...
    def zero_change(self,index): #根據路徑0的位置進行判斷，判斷方法為當下0的位置與下個0的位置是否差距n個位置，n的公式為(巡邏點總數 / 車數)
    #index 為標記該路徑0的位置
...
############################################################################
    ...
    def _crossover(self, elites): #交配
        ...
        for _ in range(self.populations - self.mutates):
            ...
            next_route_path = mother_gene[:mother_gene_cut] + father_gene + mother_gene[mother_gene_cut:] #下代基因為 母代切割 + 父代 + 母代切割
            ...
            #start_2 = time.time()
            i,check = self.zero_change(index) #計算0的位置是否符合規則
            ...
            while check == True or index[0] <= 3: #index[0] <= n n為需求數 (需求數如果有更改車輛數量或點數量需進行更改)                    
                ...
                i,check = self.zero_change(index) #再進行一次計算0的位置是否符合規則，如不符合 則替換到符合規則為止
...
if __name__ == '__main__':
    ...
    my_algo = GeneticAlgo(my_locs, level= 3, populations=50, variant=4, mutate_percent=0.1, elite_save_percent=0.2,crossover_rate = 0.6)
    ...
    export_var("best_file.txt",best_F1,best_F2,best_F3,best_path)
    export_var("all_best_f1.txt",all_best_f1)
    export_var("all_best_f2.txt",all_best_f2)
    export_var("all_best_f3.txt",all_best_f3)
