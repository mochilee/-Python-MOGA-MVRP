# -*- coding: utf-8 -*-
"""
Created on Tue Jul 26 11:51:52 2022
...
def mutate(path):
    ...
    return path 
'''
def crossover(all_path):
    next_route = []
    for i in range(25):
        father, mother = rd.choices(all_path[:15], k=2)  #我們從菁英路徑們當中的前四名隨機取出兩個作為父和母。          
        index_start = rd.randrange(0, len(father) - 4 - 1)

        father_gene = father[index_start: index_start + 4] #由菁英路徑由指定切割位置取出父代的基因            
        ...
        next_route.append(repair(next_route_path))
    return next_route
'''   
def crossover(all_path):
    next_route = []
    while len(next_route) < 25:
        father, mother = rd.choices(all_path[:15], k=2) 
        cross = rd.randint(1, 10)
        if cross == 1:
           index_start = rd.randrange(0, len(father) - 4 - 1)
...
def city_change(all_path):
    next_route = []
    for j in all_path:       
        copy_father = copy.deepcopy(j)
        ...
        next_route.append(repair(copy_father)) 
...
class EvolutionStrategy:
    ...
    def make_kid(self,all_parent_route):
        all_route = []
        #path = self._find_path()

        '''
        while len(all_route) < 25:
            rand = rd.randrange(1, 10) 
            if rand == 1:
                rand = rd.randrange(0, len(all_parent_route)) 
                path = mutate(all_parent_route[rand])
                all_route.append(path)
                '''

        all_route = city_change(all_parent_route)
        '''
        for i in all_parent_route:         
            path = mutate(i)
            all_route.append(path)
            '''
    ...
    def evolution(self):
        ...
        for i in range(25):           
            ...
            all_parent_route.append(parent)
        ...
        for _ in range(self.level):
            ...
            all_kid_route = []
            if _ < 1:
                all_kid_route = self.make_kid(all_parent_route)
            else:
                all_parent_route = []
                for i in next_parent:
                    all_parent_route.append(i.path) 
                all_kid_route = self.make_kid(all_parent_route)


            start_parent = time.time()

            next_parent = self.kill_kid(all_parent_route,all_kid_route)  

            #print(f'after next = {len(next_parent)}')
            end_parent = time.time()

            parent_cost = end_parent - start_parent
            ...
            if ( _ + 1) % 1 == 0:
                ...
                for i in all_parent_route:
                    temp.append(Route(i))
                test1_end = time.time()
                test1_cost = test1_end - test1_start
                '''

                    
                '''        
                pqc = []
                pqc= fitness(next_parent)
                print(pqc)
                

                for i in range(len(next_parent)):
                    next_parent[i].gpsiff = pqc[0][i]
                    next_parent[i].q = pqc[1][i]
                '''    
                next_parent.sort(key=lambda x: x.gpsiff, reverse=True) 

                all_path.append(next_parent[0])

                for i in range(len(next_parent)):
                    if next_parent[i].q == 0:
                        third_pop.append(next_parent[i])
                if ( _ + 1) % 100 == 0:
                    third_pop_cal(_) 
            ...
            print(f'level = {_} cost = {end - start} parent = {parent_cost}')
            '''    
        routes = []
        for i in next_parent.path:        
            routes.append(Route(i))
        pqc = []
            '''
        #pqc= fitness(routes) #計算gpsiff
        #pqc= fitness(next_parent) #計算gpsiff
        #print(pqc)
        #print(f'pqc = Pqc{pqc}')
        test = []
        for i in next_parent:
            test.append(i.gpsiff)
        print(test)
        '''
        for i in range(len(next_parent)):
            next_parent[i].gpsiff = pqc[0][i]
        ...
        print("===================================================================================")
        return next_parent[0].path, next_parent[0].length, next_parent[0].safe_value, next_parent[0].resilience
...
def export_var(number,file_name,var1 = [],var2 = [],var3 = [],var4 = [],var5 = []): #結束輸出各極限值 txt檔
    ...
    if file_name == "best_file":
        #path,name = output_path_name_(0,var1,0)
        path = [(loc.loc[0], loc.loc[1]) for loc in var1]
        name = [(loc.name) for loc in var1]
        bestAll = [path,name,var2,var3,var4]

        bestAll_txt = repr(bestAll)
        file1.write("bestAll = " + bestAll_txt + "\n")
        
    elif file_name == "all_best":  
        for i in range(len(var1)): 
            path = [(loc.loc[0], loc.loc[1]) for loc in var1[i].path]
            name = [(loc.name) for loc in var1[i].path]       
            allbestF1 = [var1[i].length,var1[i].resilience,var1[i].safe_value,path,name]
            txt = repr(allbestF1)
            file1.write(txt + "\n")
...
if __name__ == '__main__':
    ...
    my_es = EvolutionStrategy(my_locs, level= 10000)
    ...
    #print([loc.name for loc in best_route], best_route_length,best_route_safe_value,best_route_resilience)
   # print([(loc.loc[0], loc.loc[1]) for loc in best_route])
    #print([(loc.name) for loc in best_route])
    ...
    '''
    export_var(number,"best_file",best_route,best_route_length,best_route_resilience,best_route_safe_value)
    export_var(number,"all_best",all_path)
    print(f'cost time {end - start}')
    print(f'best_route_length = {best_route_length}')
    print(f'best_route_safe_value = {best_route_safe_value}')
    print(f'best_route_resilience = {best_route_resilience}')
    '''    
